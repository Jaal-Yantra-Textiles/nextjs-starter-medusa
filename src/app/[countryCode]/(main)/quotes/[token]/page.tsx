...
export default async function QuotePage({ params, searchParams }: Props) {
   ...
   * enough to put an AU buyer on `/in`. From there the accept below pushes to
   ...
   * the very state this is here to prevent. Where it does not, the current
   * prefix is left alone and the accept panel is told the truth via
   * `checkoutCountryCode` instead.
   */
  ...
  return (
    <QuoteTemplate
      ...
      // this storefront has no region for. Checkout is then still wrong for the
      // buyer, but it is wrong in the way the storefront actually supports
      // rather than in a prefix that would bounce to the default region.
